...
def convert_chars_line_to_boolean_vector(chars_line):
    ...
    if len(chars_array) != conf.chars_in_line:
        raise ValueError("line's length %s is invalid, it should be %s" % (str(len(chars_array)), str(conf.chars_in_line)))
...
def parse_invoice_numbers(file_lines):
    ...
    for line in file_lines:
        ...
        line = line.rstrip('\n')
        if line:
            if len(line) != conf.chars_in_line:
                raise ValueError("Line's length must be  %s" % str(conf.digit_size))
            ...
            if len(segment_vector_array) != conf.digit_size:
                raise ValueError("Invalid format of invoice number lines")
